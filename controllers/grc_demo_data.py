...
def _load_demo_data():
    ...
    db.maturity_level.update_or_insert((db.maturity_level.id==2), id=2, name="Performed (Ejecutado)",     m_level=1, description="The implemented process achieves its process purpose. COBIT PAM 1 attribute. (El proceso implementado logra su propósito de proceso. COBIT PAM 1 atributo.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.maturity_level.update_or_insert((db.maturity_level.id==3), id=3, name="Managed (Gestionado)",      m_level=2, description="The previously described performed process is now implemented in a managed fashion. Planned, monitored and adjusted. Its work products are appropriately established, controlled and maintained. COBIT PAM 2 attributes. (El proceso realizado descrito anteriormente se implementa ahora de manera administrada. Monitoreada, planificada y ajustada. Sus productos de trabajo se establecen, controlan y mantienen adecuadamente. COBIT PAM 2 atributos.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.maturity_level.update_or_insert((db.maturity_level.id==4), id=4, name="Established (Establecido)", m_level=3, description="The previously described managed process is now implemented using a defined process that is capable of achieving its process outcomes. COBIT PAM 2 attributes. (El proceso gestionado descrito anteriormente ahora se implementa utilizando un proceso definido que es capaz de lograr los resultados del proceso. COBIT PAM 2 atributos.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.maturity_level.update_or_insert((db.maturity_level.id==5), id=5, name="Predictable (Predecible)",  m_level=4, description="The previously described established process now operates within defined limits to achieve its process outcomes. COBIT PAM 2 attributes. (El proceso establecido descrito anteriormente ahora opera dentro de límites definidos para lograr los resultados del proceso. COBIT PAM 2 atributos.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.maturity_level.update_or_insert((db.maturity_level.id==6), id=6, name="Optimazing (Optimizado)",   m_level=5, description="The previously described predictable process is continuously improved to meet relevant current and projected business goals. COBIT PAM 2 attributes. (El proceso predecible descrito anteriormente se mejora continuamente para cumplir con los objetivos comerciales actuales y proyectados relevantes. COBIT PAM 2 atributos.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    #---------------
    # Objetive Type
    #---------------
    db.objective_type.update_or_insert((db.objective_type.id==1), id=1, name="Operational (Operacional)", description="It refers to the effectiveness and efficiency of the organization's operations, including its performance, financial and operational objectives and the protection of its assets against any possible losses. (Garantizar operaciones efectivas y eficaces. Hacen referencia a la eficacia y eficiencia de las operaciones de la organización, incluidos sus objetivos de desempeño, financieros y operativos, y la protección de sus activos frente a posibles perdidas.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.objective_type.update_or_insert((db.objective_type.id==2), id=2, name="Reporting (Información)",   description="To produce sufficient and reliable financial internal and external information in a transparent manner according to the concepts established by the regulators, organisms and/or by the own policies of the Company. (Generar información financiera y no financiera suficiente y confiable. Hacen referencia a la información financiera y no financiera interna y externa, y pueden abarcar aspectos de fiabilidad, oportunidad, transparencia u otros conceptos establecidos por los reguladores, organismos de normalización o por las políticas de la propia organización.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.objective_type.update_or_insert((db.objective_type.id==3), id=3, name="Compliance (Cumplimiento)", description="It refers to the compliance of the laws and regulations to which the organization is subjected. (Cumplir con las regulaciones aplicables, tanto internas como externas. Hacen referencia al cumplimiento de las leyes y regulaciones a las que está sujeta la organización.)", create_date= datetime.datetime.now(), risk_manager_approval='T')
    ...
    # Company
    #----------
    db.company.update_or_insert((db.company.id==1), id=1, name="The Bank", description="Accept deposits and make loans.", mision="Be the best bank in the world.", vision="Be the best bank in the world.", c_value="Loyalty & trust.", security_requirement="PCI, GDPR", framework_used="ISO27001, COSO", product_service="Credit cards", create_date= datetime.datetime.now(), risk_manager_approval='T')
    #----------------
    # Impact Metrics
    #----------------
    db.impact_level.update_or_insert((db.impact_level.id==1), id=1, name="Insignificant (Insignificante)", i_level=1, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.impact_level.update_or_insert((db.impact_level.id==2), id=2, name="Minor (Menor)",                  i_level=2, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.impact_level.update_or_insert((db.impact_level.id==3), id=3, name="Moderate (Moderado)",            i_level=3, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.impact_level.update_or_insert((db.impact_level.id==4), id=4, name="Major (Mayor)",                  i_level=4, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.impact_level.update_or_insert((db.impact_level.id==5), id=5, name="Catastrophic (Catastrofico)",    i_level=5, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    #----------------
    # Impact Metrics
    #----------------
    db.probability_level.update_or_insert((db.probability_level.id==1), id=1, name="Rare (Remoto)",                p_level=1, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.probability_level.update_or_insert((db.probability_level.id==2), id=2, name="Unlikely (Poco Probable)",     p_level=2, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.probability_level.update_or_insert((db.probability_level.id==3), id=3, name="Possible (Factible)",          p_level=3, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.probability_level.update_or_insert((db.probability_level.id==4), id=4, name="Likely (Muy Probable)",        p_level=4, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    db.probability_level.update_or_insert((db.probability_level.id==5), id=5, name="Almost Certain (Casi Cierto)", p_level=5, description="", create_date= datetime.datetime.now(), risk_manager_approval='T')
    #-------------------
    # Company Objective
    #-------------------
    db.company_objective.update_or_insert((db.company_objective.id==1), id=1, name="Compliance ISO, PCI, NIST (Cumplir con leyes y regulaciones LFPDPPP, SOX, NOM35, PCI, ISO)", objective_type=1, description="", risk_manager_approval='T')
    db.company_objective.update_or_insert((db.company_objective.id==2), id=2, name="PII protection (Proteger datos personales de clientes)",                                     objective_type=2, description="", risk_manager_approval='T')
    db.company_objective.update_or_insert((db.company_objective.id==3), id=3, name="Internal security requirements (Cumplir con los requerimientos de la Dirección General)",    objective_type=3, description="", risk_manager_approval='T')
    db.company_objective.update_or_insert((db.company_objective.id==4), id=4, name="99% IT infrastructure availability (Asegurar la disponibilidad de los sistemas al 99.99%)",  objective_type=4, description="", risk_manager_approval='T')
